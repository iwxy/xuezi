...
$('.mask').click(
		function() {
			if ($(this).children().hasClass('maskNormal')) {
				$(this).children().addClass('maskTrue').removeClass(
						'maskNormal');
				$(this).children().children().attr('src',
						'../images//myCollect/product_true_big.png');
			} else {
				$(this).children().addClass('maskNormal').removeClass(
						'maskTrue');
				$(this).children().children().attr('src',
						'../images/myCollect/product_normal_big.png');
			}
		})
// 全选
$('.all')
				...
				function() {
					if ($('.all span').hasClass('normal')) {
						...
						$('.all>span>img').attr('src',
								'../images/myCollect/product_true.png');
						$(".mask>div")
								.each(
										function() {
											$(this).addClass('maskTrue')
													.removeClass('maskNormal');
											$(this)
													.children('img')
													.attr('src',
															'../images/myCollect/product_true_big.png');
						...
						$('.all>span>img').attr('src',
								'../images/myCollect/product_normal.png');
						$(".mask>div")
								.each(
										function() {
											$(this).addClass('maskNormal')
													.removeClass('maskTrue');
											$(this)
													.children('img')
													.attr('src',
															'../images/myCollect/product_normal_big.png');
...
$('.del').click(function() {
	...
	if (str.length > 0) {
		...
		$('.yes').click(function() {
			...
			$.ajax({
				type : "POST",
				url : "../favorites/delFavorites.do",
				data : "itemIds=" + str,
				success : function(obj) {
					if (obj.state == 1) {
						window.location.href = "../favorites/showFavorites.do";
					} else {
						$('.modal').hide();
					}
				}
	...
	if (str.length > 0) {
		...
		$.ajax({
			...
			success : function(data) {
				if (data == '200') {
					/*
					 * $('.mask').hide(); $(this).next().show();
					 * setTimeout(function(){
					 * window.location.href="/toMyCollect.html"; },1000);
					 */
					alert("加入购物车成功！");
